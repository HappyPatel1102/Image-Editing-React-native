...
function ShowImage({ route, navigation }) {
    const [iamgepath, setImagepath] = useState('')
    ...
    const getImage = () => {
        ...
        RNFS.readFile(route.params.ImagePath, "base64").then(
            res => {
                setBase64Img(res)
            }
        );
    ...
    return (
        <View style={styles.container}>
            <Image
                source={iamgepath ? { uri: iamgepath } : null}
                style={styles.selectImage}
            />
            <TouchableOpacity onPress={() => navigation.goBack()} style={[styles.topIcon, { left: 10 }]} >
                <Icon
                    name="arrow-back"
                    color="#fff"
                    size={30}
            ...
            <TouchableOpacity
                onPress={() => editImage(route.params.ImagePath)}
                style={[styles.topIcon, { right: 25 }]} >
                <Icon
                    name="edit"
                    color="#fff"
                    size={30}
